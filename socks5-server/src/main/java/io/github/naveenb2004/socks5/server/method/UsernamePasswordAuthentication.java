...
public final class UsernamePasswordAuthentication implements SOCKS5ServerMethod {
    ...
    public void negotiateAsServer(InputStream inputStream,
        ...
        try {
            ...
            if (subNegotiationVer != 0x01) throw new SOCKS5ServerException("Invalid negotiation version");

            int unameLen = inputStream.read();
            byte[] unameBytes = new byte[unameLen];
            int readUlen = inputStream.read(unameBytes);
            if (unameLen != readUlen) throw new SOCKS5ServerException("Error reading negotiation data");

            int passwdLen = inputStream.read();
            byte[] passwdBytes = new byte[passwdLen];
            int readPasswdLen = inputStream.read(passwdBytes);
            if (passwdLen != readPasswdLen) throw new SOCKS5ServerException("Error reading negotiation data");
            ...
            if (!clientPasswd.equals(passwd)) {
                ...
                outputStream.write(0x01);
                outputStream.flush();
                throw new SOCKS5ServerException("Invalid username/password");
            }
    ...
    public static final class UsernamePasswordAuthenticationBuilder {
        ...
        public void addUnamePasswd(String uname,
                                   String passwd) throws SOCKS5ServerConfigException {
            if (uname == null || uname.isBlank()) {
                throw new SOCKS5ServerConfigException("Username cannot be null or blank");
            }
            if (passwd == null) throw new SOCKS5ServerConfigException("Password cannot be null");
            if (uname.length() > 255) throw new SOCKS5ServerConfigException("Username too long");
            if (passwd.length() > 255) throw new SOCKS5ServerConfigException("Password too long");
            unamePsswdMap.put(uname, passwd);
        }

        public SOCKS5ServerMethod build() throws SOCKS5ServerConfigException {
            if (this.unamePsswdMap.isEmpty()) throw new SOCKS5ServerConfigException("No username and password provided");
